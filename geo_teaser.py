...
class Vocab:
    def __init__(self, fi, pair, comb, min_count, percentage):
        ...
        print
        'num_poi_track: ', len(self.poi_track)
        print
        'num_poi_time_track', len(self.poi_time_track)
        ...
        self.rating_count
        print
        self.rating_count * percentage
        self.split(percentage)
        # assert self.word_count == sum([t.count for t in self.vocab_items]), 'word_count and sum of t.count do not agree'
        print
        'Total user in training file: %d' % self.user_count
        print
        'Total item in training file: %d' % self.item_count
        print
        'Total rating in file: %d' % self.rating_count
        print
        'Total POI tracking (day): %d' % len(self.poi_track.keys())
        print
        len(poi_list)
    ...
    while start < end:
        ...
        for sent_pos, token in enumerate(sent):
            ...
            for x in range((len(vocab.vocab_items[c_user].item_set.keys()))):
                neighbor_item = np.random.choice(list(vocab.vocab_items[c_user].item_set.keys()))
                non_neighbors = []
...
def train(fi, pair, comb, split, fo, cbow, neg, dim, alpha, beta, win, min_count, num_processes, binary,
    ...
    pool = Pool(processes=num_processes, initializer=__init_process,
                initargs=(vocab, syn0, syn_user, syn1, syn_t, table, cbow, neg, dim, alpha, beta,
                          win, num_processes, global_word_count, last_error, current_error, num_non_neighbors,
                          neighbor_threshold, fi))
    ...
    result = Manager().dict()
    pool = Pool(processes=num_processes, initializer=__init_evaluation,
    ...
    e_time = time.time() - t2
    print
    'complete evalution, cost ', str(e_time / 60)
    for u in result.keys():
        ...
        r10 += float(a[3])
    print
    '#test: %d, p@5: %f, p@10: %f, r@5: %f, r@10: %f\n' % (
        ...
        if user in vocab.test_data:
            ...
            if len(wday_0):
                if len(wday_1):
                    ...
                    top10_0 = list(ranked_0.keys())[:10]
                    top10_1 = list(ranked_1.keys())[:10]
                    top5_0 = list(ranked_0.keys())[:5]
                    top5_1 = list(ranked_1.keys())[:5]
                    a = int(rate_0 * 10)
                    b = 10 - a
...
def predict(vocab, syn0, syn_user, syn_t):
    ...
    for user in vocab.test_data:
        ...
        # print 'Cost: ', t2-t1
    print
    'P@5, P@10, R@5, R@10', p5 / test_case, p10 / test_case, r5 / test_case, r10 / test_case
    ofile = open('result.txt', 'w')
